...
module.exports = function(file, path, command){
    ...
    try {
        ...
        var outname = command.out || Path.basename(file, Path.extname(file)) + '.png';
        var format = command.format || Path.extname(outname);
        
        if(format !== "png" && format!== "jpg" && format !== ".png" && format!== ".jpg"){
            throw new Error(`Specified format (${format}) is not supported. Acceptable formats are: png, jpg`);
        }
        
        var outfile = Path.join(path, outname);
        
        if (format === "png" && Path.extname(outname) !== ".png"){
            throw new Error(`Output filename (${outname}) does not have ${format} extension`);
        }
        else if (format === "jpg" && Path.extname(outname) !== ".jpg"){
            throw new Error(`Output filename (${outname}) does not have ${format} extension`);
        }
        
        return new Ffmpeg(file).thumbnail(outfile)
