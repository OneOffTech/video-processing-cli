...
const FFMPEG_DOWNLOAD_URL = {
    "win": "https://ffmpeg.zeranoe.com/builds/win{architecture}/static/ffmpeg-{version}-win{architecture}-static.zip",
    // "macos": ["https://evermeet.cx/ffmpeg/ffmpeg-{version}.7z", "https://evermeet.cx/ffmpeg/ffprobe-{version}.7z"],
    "linux": "https://johnvansickle.com/ffmpeg/releases/ffmpeg-{version}-{architecture}bit-static.tar.xz"
};

const SHAKA_PACKAGER_DOWNLOAD_URL = {
    "win": "https://github.com/google/shaka-packager/releases/download/v{version}/packager-win.exe",
    "macos": "https://github.com/google/shaka-packager/releases/download/v{version}/packager-osx",
    "linux": "https://github.com/google/shaka-packager/releases/download/v{version}/packager-linux"
};
...
function downloadShakaPackager(platform){
    var packagerUrl = SHAKA_PACKAGER_DOWNLOAD_URL[platform]
        .replace(/\{version\}/g, Package.binaries.packager);
...
function downloadFfmpeg(platform, architecture){
    ...
    if(!FFMPEG_DOWNLOAD_URL[platform]){
        Log.error('FFMPEG download aborted: platform not supported', "("+platform+")");
    ...
    .then(function(f){
        ...
        return platform==='win' ? extractFfmpegWindows('./bin/' + Path.basename(ffmpegUrl), './bin/') : extractFfmpegLinux(Path.join(process.cwd(), 'bin', Path.basename(ffmpegUrl)), Path.join(process.cwd(), 'bin'));
...
function extractFfmpegWindows(file, path){
    ...
    return new Promise(function(resolve, reject){
        ...
        unzip(file, path, function(err){
            ...
            fse.copySync(Path.join(path, Path.basename(file).replace(Path.extname(file),''), 'bin', 'ffmpeg.exe'), Path.join(path, 'ffmpeg.exe'), { overwrite: true })
            fse.copySync(Path.join(path, Path.basename(file).replace(Path.extname(file),''), 'bin', 'ffprobe.exe'), Path.join(path, 'ffprobe.exe'), { overwrite: true })

            fse.removeSync(Path.join(path, Path.basename(file).replace(Path.extname(file),'')));
            fse.removeSync(file);
            resolve(null);
        })
    }).then(function(){
...
function extractFfmpegLinux(file, path){
    ...
    return new Promise(function(resolve, reject){
        ...
        var untar = exec(command, (err, stdout, stderr) => {
            ...
            fse.copySync(Path.join(path, Path.basename(file).replace('.tar.xz',''), 'ffmpeg'), Path.join(path, 'ffmpeg'), { overwrite: true })
            fse.copySync(Path.join(path, Path.basename(file).replace('.tar.xz',''), 'ffprobe'), Path.join(path, 'ffprobe'), { overwrite: true })

            fse.removeSync(Path.join(path, Path.basename(file).replace('.tar.xz','')));
            fse.removeSync(file);
            resolve(null);
        });
...
module.exports = function(command){
    ...
    try {
        const platformNormalized = process.platform.match(/win(32|64)/) ? "win" : (process.platform.match(/darwin/) ? "macos" : "linux");
        ...
        Log.comment("Downloading binaries", "(" + platformNormalized + ", " + architectureNormalized + " bit)...");
